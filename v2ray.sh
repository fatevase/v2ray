...
v2ray_client_config="/etc/v2ray/server_v2ray_config.json"
backup="/etc/v2ray/server_v2ray_backup.conf"
...
_load() {
	local _dir="/etc/v2ray/server/v2ray/src/"
	. "${_dir}$@"
}
_sys_timezone() {
	IS_OPENVZ=
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta} 用来分流的路径 $none , 例如 /fakeline , 那么只需要输入 fakeline 即可"
		read -p "$(echo -e "(默认: [${cyan}fakeline$none]):")" path
		[[ -z $path ]] && path="fakeline"
...
proxy_site_config() {
	echo
	while :; do
		echo -e "请输入 ${magenta}一个正确的 $none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://baidu.com"
		echo -e "举例...你当前的域名是 $green$domain$none , 伪装的网址的是 https://baidu.com"
		echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://baidu.com 的内容(反向代理)"
		echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		read -p "$(echo -e "(默认: [${cyan}https://baidu.com$none]):")" proxy_site
		[[ -z $proxy_site ]] && proxy_site="https://baidu.com"
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}password123$none)"): " sspass
		[ -z "$sspass" ] && sspass="password123"
		case $sspass in
		*[/$]*)
			echo
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			...
			echo
			echo -e " 请确保你有完整的 V2Ray 一键安装脚本 & 管理脚本到当前 ${green}$(pwd) $none目录下"
			echo
			exit 1
		fi
		mkdir -p /etc/v2ray/server/v2ray
		cp -rf $(pwd)/* /etc/v2ray/server/v2ray
	else
		pushd /tmp
		# when install failed, try to reclone this script
		git clone https://github.com/fatevase/v2ray -b "$_gitbranch" /etc/v2ray/server/v2ray --depth=1
		popd

	fi

	if [[ ! -d /etc/v2ray/server/v2ray ]]; then
		echo
...
config() {
	cp -f /etc/v2ray/server/v2ray/config/backup.conf $backup
	cp -f /etc/v2ray/server/v2ray/v2ray.sh $_v2ray_sh
	chmod +x $_v2ray_sh
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=password123/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] || $v2ray_transport == 33 ]] && sed -i "36s/=google.com/=$domain/" $backup
	[[ $caddy ]] && sed -i "39s/=/=true/" $backup
	[[ $ban_ad ]] && sed -i "54s/=/=true/" $backup
	if [[ $is_path ]]; then
		sed -i "57s/=/=true/; 60s/=server/=$path/" $backup
		sed -i "63s#=https://baidu.com#=$proxy_site#" $backup
	fi
}
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/server/v2ray ]]; then
		echo
		...
		echo
		exit 1
	elif [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f /etc/v2ray/server_v2ray_backup.txt && -d /etc/v2ray/server/v2ray ]]; then
		echo
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/server/v2ray ]]; then
		...
		备注...仅支持卸载使用 同脚本 提供的 V2Ray 一键安装脚本
		" && exit 1
	fi
...
*)
	...
	echo
	echo -e " 脚本仅支持输入$green local / online $none参数"
	echo
